...
export function TicketQueue({ categoryId, tickets, onTicketClick, onBack }: TicketQueueProps) {
  ...
  const getStatusBadge = (status: QueueTicketItem['status']) => {
    const badges = {
    ...
    <div className="flex flex-col h-full">
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex-shrink-0 flex flex-col items-end gap-2">
                <span className={`font-label text-[9px] px-2 py-0.5 rounded ${getStatusBadge(ticket.status).bg} ${getStatusBadge(ticket.status).text}`}>
      ...
      <footer className="px-4 py-3 bg-zinc-50 border-t border-outline-variant flex-shrink-0">
        <div className="flex justify-between items-center">
          ...
          <span className="font-label text-[10px] text-zinc-400">
            {tickets.filter(t => t.status === 'new').length} New · {tickets.filter(t => t.status === 'drafted').length} Review
          </span>
        </div>
      </footer>
    </div>
